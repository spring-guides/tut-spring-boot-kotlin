...
Sometimes you need to execute a method before or after all tests of a given class. JUnit requires by default these methods to be static (which translates to https://kotlinlang.org/docs/object-declarations.html#companion-objects[`companion object`] in Kotlin, which is quite verbose and not straightforward) because test classes are instantiated once per test.

But JUnit allows you to change this default behavior and instantiate test classes once per class. This can be done in https://docs.junit.org/current/writing-tests/test-instance-lifecycle.html[various ways]. Here we will use a property file to change the default behavior for the whole project:
...
With this configuration, we can now use the `@BeforeAll` and `@AfterAll` annotations on regular methods like shown in the updated version of `IntegrationTests` below.
