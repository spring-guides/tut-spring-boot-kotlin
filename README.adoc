...
Although Java does not allow one to express null-safety in its type-system, Spring Framework provides null-safety of the whole Spring Framework API via tooling-friendly annotations declared in the `org.springframework.lang` package. By default, types from Java APIs used in Kotlin are recognized as https://kotlinlang.org/docs/java-interop.html#null-safety-and-platform-types[platform types] for which null-checks are relaxed. https://kotlinlang.org/docs/java-interop.html#jsr-305-support[Kotlin support for JSR 305 annotations] + Spring nullability annotations provide null-safety for the whole Spring Framework API to Kotlin developers, with the advantage of dealing with `null` related issues at compile time.
...
2 Kotlin specific libraries are required (the standard library is added automatically with Gradle) for such a Spring Boot web application and configured by default:
...
In addition to the obvious https://kotlinlang.org/docs/maven.html[Kotlin Maven plugin], the default configuration declares the https://kotlinlang.org/docs/all-open-plugin.html[kotlin-spring plugin] which automatically opens classes and methods (unlike in Java, the default qualifier is `final` in Kotlin) annotated or meta-annotated with Spring annotations. This is useful to be able to create `@Configuration` or `@Transactional` beans without having to add the `open` qualifier required by CGLIB proxies for example.
...
Although Java does not allow one to express null-safety in its type-system, Spring Framework provides null safety of the whole Spring Framework API via tooling-friendly https://jspecify.dev/[JSpecify] annotations. Kotlin support for JSpecify annotations, and Spring nullability annotations provide null safety for the whole Spring Framework API to Kotlin developers with the advantage of dealing with `null` related issues at compile time.

=== Dependencies

Two Kotlin-specific libraries are required for such a Spring Boot web application and configured by default:
